...
## 34. Artifact 的“打开”和“下载文件”执行相同行为

- 问题：DOCX Artifact 卡片同时显示“打开”和“下载文件”，但两个链接都指向带附件响应头的下载接口，因此点击后都会下载。
- 根因：系统没有 HTML/PDF 预览转换链，也没有可嵌入浏览器的 Office Viewer；前端仅通过 `target=_blank` 包装同一个下载 URL，造成虚假的功能区分。
- 修复：移除“打开”，只保留一个语义与实际行为一致的“下载文件”按钮；同时移除相邻的模型工程标签。
- 当前状态：PASS。Artifact 下载协议与历史恢复不变，没有引入第三方在线文档查看服务或文件外传。

## 35. 用户消息“编辑”跳到页面底部且无法保留版本

- 问题：旧实现点击“编辑”后把文字装入底部 Composer；用户失去当前消息上下文，重新发送也只形成普通后续问题，没有 `1 / 2` 版本选择。
- 根因：Message 仅按创建时间线性保存，没有 parent、revision group 或 active leaf；ChatMemory 也始终恢复全部历史，无法表达分支。
